/*
Proxmox

No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

API version: 2023-01-01
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package proxmox

import (
	"encoding/json"
)

// checks if the GetSmartHealthResponseContent type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &GetSmartHealthResponseContent{}

// GetSmartHealthResponseContent struct for GetSmartHealthResponseContent
type GetSmartHealthResponseContent struct {
	Data []DiskSummary `json:"data"`
}

// NewGetSmartHealthResponseContent instantiates a new GetSmartHealthResponseContent object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewGetSmartHealthResponseContent(data []DiskSummary) *GetSmartHealthResponseContent {
	this := GetSmartHealthResponseContent{}
	this.Data = data
	return &this
}

// NewGetSmartHealthResponseContentWithDefaults instantiates a new GetSmartHealthResponseContent object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewGetSmartHealthResponseContentWithDefaults() *GetSmartHealthResponseContent {
	this := GetSmartHealthResponseContent{}
	return &this
}

// GetData returns the Data field value
func (o *GetSmartHealthResponseContent) GetData() []DiskSummary {
	if o == nil {
		var ret []DiskSummary
		return ret
	}

	return o.Data
}

// GetDataOk returns a tuple with the Data field value
// and a boolean to check if the value has been set.
func (o *GetSmartHealthResponseContent) GetDataOk() ([]DiskSummary, bool) {
	if o == nil {
		return nil, false
	}
	return o.Data, true
}

// SetData sets field value
func (o *GetSmartHealthResponseContent) SetData(v []DiskSummary) {
	o.Data = v
}

func (o GetSmartHealthResponseContent) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o GetSmartHealthResponseContent) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["data"] = o.Data
	return toSerialize, nil
}

type NullableGetSmartHealthResponseContent struct {
	value *GetSmartHealthResponseContent
	isSet bool
}

func (v NullableGetSmartHealthResponseContent) Get() *GetSmartHealthResponseContent {
	return v.value
}

func (v *NullableGetSmartHealthResponseContent) Set(val *GetSmartHealthResponseContent) {
	v.value = val
	v.isSet = true
}

func (v NullableGetSmartHealthResponseContent) IsSet() bool {
	return v.isSet
}

func (v *NullableGetSmartHealthResponseContent) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableGetSmartHealthResponseContent(val *GetSmartHealthResponseContent) *NullableGetSmartHealthResponseContent {
	return &NullableGetSmartHealthResponseContent{value: val, isSet: true}
}

func (v NullableGetSmartHealthResponseContent) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableGetSmartHealthResponseContent) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


