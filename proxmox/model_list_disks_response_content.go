/*
Proxmox

No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

API version: 2023-01-01
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package proxmox

import (
	"encoding/json"
)

// checks if the ListDisksResponseContent type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &ListDisksResponseContent{}

// ListDisksResponseContent struct for ListDisksResponseContent
type ListDisksResponseContent struct {
	Data []DiskSummary `json:"data"`
}

// NewListDisksResponseContent instantiates a new ListDisksResponseContent object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewListDisksResponseContent(data []DiskSummary) *ListDisksResponseContent {
	this := ListDisksResponseContent{}
	this.Data = data
	return &this
}

// NewListDisksResponseContentWithDefaults instantiates a new ListDisksResponseContent object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewListDisksResponseContentWithDefaults() *ListDisksResponseContent {
	this := ListDisksResponseContent{}
	return &this
}

// GetData returns the Data field value
func (o *ListDisksResponseContent) GetData() []DiskSummary {
	if o == nil {
		var ret []DiskSummary
		return ret
	}

	return o.Data
}

// GetDataOk returns a tuple with the Data field value
// and a boolean to check if the value has been set.
func (o *ListDisksResponseContent) GetDataOk() ([]DiskSummary, bool) {
	if o == nil {
		return nil, false
	}
	return o.Data, true
}

// SetData sets field value
func (o *ListDisksResponseContent) SetData(v []DiskSummary) {
	o.Data = v
}

func (o ListDisksResponseContent) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o ListDisksResponseContent) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["data"] = o.Data
	return toSerialize, nil
}

type NullableListDisksResponseContent struct {
	value *ListDisksResponseContent
	isSet bool
}

func (v NullableListDisksResponseContent) Get() *ListDisksResponseContent {
	return v.value
}

func (v *NullableListDisksResponseContent) Set(val *ListDisksResponseContent) {
	v.value = val
	v.isSet = true
}

func (v NullableListDisksResponseContent) IsSet() bool {
	return v.isSet
}

func (v *NullableListDisksResponseContent) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableListDisksResponseContent(val *ListDisksResponseContent) *NullableListDisksResponseContent {
	return &NullableListDisksResponseContent{value: val, isSet: true}
}

func (v NullableListDisksResponseContent) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableListDisksResponseContent) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


