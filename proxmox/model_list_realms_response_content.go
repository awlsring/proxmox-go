/*
Proxmox

No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

API version: 2023-01-01
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package proxmox

import (
	"encoding/json"
)

// checks if the ListRealmsResponseContent type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &ListRealmsResponseContent{}

// ListRealmsResponseContent struct for ListRealmsResponseContent
type ListRealmsResponseContent struct {
	Data []RealmSummary `json:"data"`
}

// NewListRealmsResponseContent instantiates a new ListRealmsResponseContent object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewListRealmsResponseContent(data []RealmSummary) *ListRealmsResponseContent {
	this := ListRealmsResponseContent{}
	this.Data = data
	return &this
}

// NewListRealmsResponseContentWithDefaults instantiates a new ListRealmsResponseContent object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewListRealmsResponseContentWithDefaults() *ListRealmsResponseContent {
	this := ListRealmsResponseContent{}
	return &this
}

// GetData returns the Data field value
func (o *ListRealmsResponseContent) GetData() []RealmSummary {
	if o == nil {
		var ret []RealmSummary
		return ret
	}

	return o.Data
}

// GetDataOk returns a tuple with the Data field value
// and a boolean to check if the value has been set.
func (o *ListRealmsResponseContent) GetDataOk() ([]RealmSummary, bool) {
	if o == nil {
		return nil, false
	}
	return o.Data, true
}

// SetData sets field value
func (o *ListRealmsResponseContent) SetData(v []RealmSummary) {
	o.Data = v
}

func (o ListRealmsResponseContent) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o ListRealmsResponseContent) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["data"] = o.Data
	return toSerialize, nil
}

type NullableListRealmsResponseContent struct {
	value *ListRealmsResponseContent
	isSet bool
}

func (v NullableListRealmsResponseContent) Get() *ListRealmsResponseContent {
	return v.value
}

func (v *NullableListRealmsResponseContent) Set(val *ListRealmsResponseContent) {
	v.value = val
	v.isSet = true
}

func (v NullableListRealmsResponseContent) IsSet() bool {
	return v.isSet
}

func (v *NullableListRealmsResponseContent) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableListRealmsResponseContent(val *ListRealmsResponseContent) *NullableListRealmsResponseContent {
	return &NullableListRealmsResponseContent{value: val, isSet: true}
}

func (v NullableListRealmsResponseContent) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableListRealmsResponseContent) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


